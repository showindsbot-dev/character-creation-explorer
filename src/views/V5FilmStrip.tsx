            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '20px 28px', gap: 16, overflowY: 'auto' }}>
              ...
              {mode === 'text' && (
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div style={{ fontSize: 10, color: 'var(--text-tertiary)', letterSpacing: '0.12em' }}>CHARACTER BRIEF</div>
                  <textarea value={text} onChange={e => setText(e.target.value)}
                    placeholder="Who is this character? What drives them? Where do they come from?"
                    style={{
              ...
              {mode === 'image' && (
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <div style={{
                    flex: 1, minHeight: 180, border: '2px solid var(--border)', borderRadius: 12,
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <AnimatePresence mode="wait">
                    {dStep === 0 && (
                      ...
                      >
                        <div style={{ fontSize: 16, fontWeight: 500 }}>SCENE 01 · AESTHETIC</div>
                    ...
                    {dStep === 1 && (
                      <motion.div key="fd1" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}
                        style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}
                      >
                        <div style={{ fontSize: 16, fontWeight: 500 }}>SCENE 02 · ENERGY</div>
                    ...
                    {dStep === 2 && (
                      <motion.div key="fd2" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}
                        style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}
                      >
                        <div style={{ fontSize: 16, fontWeight: 500 }}>SCENE 03 · DETAILS</div>
          ...
          >
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '16px 20px', gap: 14, overflow: 'hidden' }}>
              ...
              <div ref={stripRef} style={{
                flex: 1, display: 'flex', gap: 10, overflowX: 'auto', alignItems: 'stretch',
                scrollSnapType: 'x mandatory', minHeight: 0, paddingBottom: 4,
              }}>
                {ResultCards.map((idx, i) => (
                  <motion.div key={i}
                    initial={{ opacity: 0, scale: 0.85 }} animate={{ opacity: 1, scale: i === activeCard ? 1 : 0.93 }}
                    transition={{ delay: i * 0.07, type: 'spring', stiffness: 300, damping: 24 }}
                    onClick={() => setActiveCard(i)}
                    style={{
                      flexShrink: 0, width: i === activeCard ? 200 : 170,
                      scrollSnapAlign: 'center', cursor: 'pointer', transition: 'width 0.25s ease',
                      borderRadius: 14, overflow: 'hidden',
                      border: i === activeCard ? '2px solid var(--accent-pink)' : '2px solid transparent',
                      boxShadow: i === activeCard ? '0 0 20px rgba(251,35,194,0.25)' : 'none',
                    }}
                  >
              ...
              <div style={{
                ...
                background: 'var(--bg-card)', borderRadius: 20, padding: '8px 14px',
                border: '1px solid var(--border)',
              }}>
